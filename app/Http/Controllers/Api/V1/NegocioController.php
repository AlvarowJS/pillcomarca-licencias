...
class NegocioController extends Controller
{
    ...
    {
        // Usamos 'with' para cargar las relaciones completas en lugar de solo los IDs
        $datos = Negocio::with('subcategoria', 'administrado', 'actividad_economica')->find($id);

        // Verificamos si se encontró el registro
        if (!$datos) {
            return response()->json(['message' => 'No se encontró el negocio buscado']);
        }

        // Retornamos los datos completos, incluyendo las relaciones
    ...
    public function updateFoto(Request $request)
    {
        ...
        $negocio->lote = $request->lote;
        $negocio->razonsocial = $request->razonsocial;
        $negocio->redsocial = $request->redsocial? null : '';
        $negocio->publico = $request->publico ? 0 : 1;
        $negocio->subcategoria_id = $request->subcategoria_id;
        $negocio->administrado_id = $request->administrado_id;
        $negocio->actividad_economica_id = $request->actividad_economica_id;
